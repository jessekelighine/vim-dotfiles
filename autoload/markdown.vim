...
function! markdown#ToHtml(type)
	silent write
	if     a:type == "plain"    | ! pandoc -f markdown+east_asian_line_breaks -s                                   -c ~/.config/nvim/snippets/markdown/github-dark.css % -o %:r.html
	elseif a:type == "nonumber" | ! pandoc -f markdown+east_asian_line_breaks -s --toc                   --mathjax -c ~/.config/nvim/snippets/markdown/github-dark.css % -o %:r.html
	elseif a:type == "number"   | ! pandoc -f markdown+east_asian_line_breaks -s --toc --number-sections --mathjax -c ~/.config/nvim/snippets/markdown/github-dark.css % -o %:r.html
